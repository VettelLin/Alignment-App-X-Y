    ...
    public class Pcr18Client
    {
        ...
        private void PortClient_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            lock (_receivedLock)
            {
                ...
                // 包头
                if (packLen == 0 && value.StartsWith("5E"))
                {
                    packLen = GetDataLength(value) * 2;
                }

                // 检查长度
                if (packLen > packData.Length)
                {
                    LogHelper.Debug("拆包处理。包总长: {0}", packLen);

                    return;
                }

                string packReal = packData.ToString().Substring(0, packLen);

                DataReceived?.Invoke(packReal);
        ...
        /// <summary>
        /// 设置 X 轴 Alignment 绝对目标值（一次下发 5 条：FAM/VIC/ROX/CY5/CY6）
        /// index: 0:-2.00, 1:-1.50, 2:-0.50, 3:-0.375, 4:-0.1875, 5:-0.3125, 6:-0.25, 7:0.00 (mm)
        /// 注意：需在采集光之前发送完成
        /// </summary>
        public void SetXAlignmentPreset(int index)
        {
            byte[][][] presets = new byte[][][]
            {
                ...
                new byte[][]
                {
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x00,0x1D,0x02,0x9F},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x08,0xF8,0x03,0x83},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x10,0xDC,0x05,0x71},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x18,0xBE,0x07,0x5D},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x20,0x9A,0x09,0x43},
                },
                // -1.50 mm → 572, 1047, 1531, 2013, 2489
                new byte[][]
                {
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x00,0x3C,0x02,0xBE},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x08,0x17,0x04,0xA3},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x10,0xFB,0x05,0x90},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x18,0xDD,0x07,0x7C},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x20,0xB9,0x09,0x62},
                },
                // -0.50 mm → 635, 1110, 1594, 2076, 2552
                new byte[][]
                {
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x00,0x7B,0x02,0xFD},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x08,0x56,0x04,0xE2},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x10,0x3A,0x06,0xD0},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x18,0x1C,0x08,0xBC},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x20,0xF8,0x09,0xA1},
                },
                // -0.375 mm → 643, 1118, 1602, 2084, 2560
                new byte[][]
                {
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x00,0x83,0x02,0x05},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x08,0x5E,0x04,0xEA},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x10,0x42,0x06,0xD8},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x18,0x24,0x08,0xC4},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x20,0x00,0x0A,0xAA},
                },
                // -0.1875 mm → 655, 1130, 1614, 2096, 2572
                new byte[][]
                {
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x00,0x8F,0x02,0x11},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x08,0x6A,0x04,0xF6},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x10,0x4E,0x06,0xE4},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x18,0x30,0x08,0xD0},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x20,0x0C,0x0A,0xB6},
                },
                // -0.3125 mm → 647, 1122, 1606, 2088, 2564
                new byte[][]
                {
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x00,0x87,0x02,0x09},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x08,0x62,0x04,0xEE},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x10,0x46,0x06,0xDC},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x18,0x28,0x08,0xC8},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x20,0x04,0x0A,0xAE},
                },
                // -0.25 mm → 651, 1126, 1610, 2092, 2568
                new byte[][]
                {
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x00,0x8B,0x02,0x0D},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x08,0x66,0x04,0xF2},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x10,0x4A,0x06,0xE0},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x18,0x2C,0x08,0xCC},
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x20,0x08,0x0A,0xB2},
                },
                // 0.00 mm → 667, 1142, 1626, 2108, 2584
                new byte[][]
                {
                    // FAM：5E 12 00 0A 00 06 00 9B 02 1D  -> 0x029B = 667
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x00,0x9B,0x02,0x1D},
                    // HEX(VIC)：5E 12 00 0A 00 06 08 76 04 02 -> 0x0476 = 1142
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x08,0x76,0x04,0x02},
                    // ROX：5E 12 00 0A 00 06 10 5A 06 F0 -> 0x065A = 1626
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x10,0x5A,0x06,0xF0},
                    // Cy5：5E 12 00 0A 00 06 18 3C 08 E2 -> 0x083C = 2108
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x18,0x3C,0x08,0xE2},
                    // Cy5.5(CY6)：5E 12 00 0A 00 06 20 18 0A C2 -> 0x0A18 = 2584
                    new byte[]{0x5E,0x12,0x00,0x0A,0x00,0x06,0x20,0x18,0x0A,0xC2},
                },
            };
            if (index < 0 || index >= presets.Length) return;
            foreach (var frame in presets[index])
            {
                SendData(frame, "SetXAlignmentPreset");
                Thread.Sleep(100); // 给设备更充裕的处理时间
            }
            // 一次性写入 5 条后，再额外缓冲一段时间，避免紧接着的扫描指令过早到达
            // 对 0.00 mm（index=7）给予更长的缓冲时间
            Thread.Sleep(index == 7 ? 500 : 200);
        }
        #endregion
